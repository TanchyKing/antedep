...
#'
#' @keywords internal
.rss_ad <- function(y, i, p, mu = NULL, include_intercept = TRUE) {
    n <- nrow(y)
...
#'
#' @keywords internal
.rss_vector_ad <- function(y, p, mu = NULL) {
    N <- ncol(y)
    rss <- numeric(N)

    for (i in 1:N) {
        rss[i] <- .rss_ad(y, i, p, mu = mu)
    }
...
#'
#' @keywords internal
.partial_corr_ad <- function(y, i, j, mu = NULL) {
...
#'
#' @keywords internal
.partial_corr_matrix_ad <- function(y, mu = NULL) {
    N <- ncol(y)
    R <- matrix(0, nrow = N, ncol = N)

    for (i in 2:N) {
        for (j in 1:(i-1)) {
            R[i, j] <- .partial_corr_ad(y, i, j, mu = mu)
...
#'
#' @keywords internal
.rss_two_sample_ad <- function(y, blocks, i, p, pooled = TRUE) {
    g1 <- which(blocks == 1)
    g2 <- which(blocks == 2)
    ...
    if (pooled) {
        ...
        rss1 <- .rss_ad(y[g1, , drop = FALSE], i, p, mu = NULL, include_intercept = TRUE)
        rss2 <- .rss_ad(y[g2, , drop = FALSE], i, p, mu = NULL, include_intercept = TRUE)
        rss1 + rss2
    }
}
...
#'
#' @keywords internal
.innovation_var_ad <- function(y, p, mu = NULL) {
    n <- nrow(y)
    rss <- .rss_vector_ad(y, p, mu = mu)
    rss / n
}
...
#'
#' @keywords internal
.group_means_ad <- function(y, blocks) {
    g1 <- which(blocks == 1)
    g2 <- which(blocks == 2)
...
#'
#' @keywords internal
.count_params_ad <- function(order, n_time, n_blocks = 1L, estimate_mu = FALSE) {
    k <- 0L
