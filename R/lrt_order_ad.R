...
#'
#' @return A list with class \code{ad_order_test} containing:
#' \describe{
...
#'
#' @seealso \code{\link{lrt_one_sample_ad}}, \code{\link{lrt_homogeneity_ad}}
#'
#' @examples
#' \dontrun{
#' # Simulate AD(1) data
#' y <- simulate_ad(n_subjects = 50, n_time = 6, order = 1, phi = 0.5)
#'
#' # Test AD(0) vs AD(1)
#' test01 <- lrt_order_ad(y, p = 0, q = 1)
#' print(test01)
#'
#' # Test AD(1) vs AD(2)
#' test12 <- lrt_order_ad(y, p = 1, q = 1)
#' print(test12)
#' }
#'
#' @export
lrt_order_ad <- function(y, p = 0L, q = 1L, mu = NULL, use_modified = TRUE) {
    ...
    for (j in 1:q) {
        for (i in (p + j + 1):n_time) {
            ...
            lag <- p + j
            r <- .partial_corr_ad(y, i, i - lag, mu = mu)
    ...
    if (use_modified) {
        ...
        rss_p <- .rss_vector_ad(y, p, mu = mu)
        rss_pq <- .rss_vector_ad(y, p + q, mu = mu)
...
#' @param ... Additional arguments passed to \code{\link{fit_ad}}.
#'
#' @return A list with class \code{ad_bic_order} containing:
#' \describe{
...
bic_order_ad <- function(y, max_order = 2L, ...) {
    ...
    for (ord in 0:max_order) {
        fits[[ord + 1]] <- fit_ad(y, order = ord, ...)
        bic_vals[ord + 1] <- bic_ad(fits[[ord + 1]], n)
    }
...
#'
#' @param x Object of class \code{ad_order_test}.
#' @param ... Unused.
#'
#' @export
print.ad_order_test <- function(x, ...) {
...
#'
#' @param x Object of class \code{ad_bic_order}.
#' @param ... Unused.
#'
#' @export
print.ad_bic_order <- function(x, ...) {
