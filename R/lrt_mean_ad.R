...
#'
#' @return A list with class \code{ad_mean_test} containing:
#' \describe{
...
#'
#' @seealso \code{\link{lrt_two_sample_ad}}, \code{\link{lrt_order_ad}}
#'
#' @examples
#' \dontrun{
...
lrt_one_sample_ad <- function(y, mu0, p = 1L, use_modified = TRUE) {
    ...
    rss_null <- .rss_vector_ad(y, p, mu = mu0)
    rss_alt <- .rss_vector_ad(y, p, mu = mu_hat)
...
#' n1 <- 30
#' n2 <- 35
#' y1 <- simulate_ad(n1, n_time = 6, order = 1, mu = rep(10, 6))
#' y2 <- simulate_ad(n2, n_time = 6, order = 1, mu = rep(12, 6))
#' y <- rbind(y1, y2)
#' blocks <- c(rep(1, n1), rep(2, n2))
#'
#' # Test equality of profiles
#' test <- lrt_two_sample_ad(y, blocks, p = 1)
#' print(test)
#' }
#'
#' @export
lrt_two_sample_ad <- function(y, blocks, p = 1L, use_modified = TRUE) {
    ...
    for (i in 1:n_time) {
        rss_null[i] <- .rss_ad(y, i, p, mu = mu_pooled)
    }
    ...
    for (i in 1:n_time) {
        rss1 <- .rss_ad(y[g1, , drop = FALSE], i, p, mu = mu1_hat)
        rss2 <- .rss_ad(y[g2, , drop = FALSE], i, p, mu = mu2_hat)
        rss_alt[i] <- rss1 + rss2
    }
...
#' @param p Antedependence order of the covariance structure.
#'
#' @return A list with class \code{ad_contrast_test} containing:
#' \describe{
...
#'
#' @export
lrt_contrast_ad <- function(y, C, c = NULL, p = 1L) {
    ...
    fit <- fit_ad(y, order = p, estimate_mu = FALSE)

    # Construct the AD covariance matrix from fit
    Sigma_hat <- .construct_ad_covariance(fit, n_time)
...
#'
#' @param fit Fitted AD model from fit_ad().
#' @param n_time Number of time points.
#'
#' @return Covariance matrix.
#'
#' @keywords internal
.construct_ad_covariance <- function(fit, n_time) {
...
#'
#' @param x Object of class \code{ad_mean_test}.
#' @param ... Unused.
#'
#' @export
print.ad_mean_test <- function(x, ...) {
...
#'
#' @param x Object of class \code{ad_contrast_test}.
#' @param ... Unused.
#'
#' @export
print.ad_contrast_test <- function(x, ...) {
