...
#'
#' @return List with fitted model (same structure as fit_ad, plus EM info)
#'
#' @keywords internal
.fit_ad_em <- function(y, order, blocks = NULL, estimate_mu = TRUE,
  ...
  init <- .initialize_ad_em(y, order, blocks, estimate_mu)
  mu <- init$mu
  phi <- init$phi
  sigma <- init$sigma
  tau <- init$tau
  ...
  for (iter in 1:max_iter) {
    ...
    suff_stats <- .em_e_step_ad(y, order, mu, phi, sigma, blocks, tau)
    
    # M-step: Update parameters
    params <- .em_m_step_ad(suff_stats, order, blocks, estimate_mu, n_subjects, n_time)
    mu <- params$mu
    phi <- params$phi
    sigma <- params$sigma
    tau <- params$tau
    ...
    ll <- .logL_ad_missing(y, order, mu, phi, sigma, blocks, tau)
    ll_trace[iter] <- ll
...
#'
#' @keywords internal
.initialize_ad_marginal <- function(y, order, blocks) {
  n_time <- ncol(y)
...
#'
#' @keywords internal
.em_e_step_ad <- function(y, order, mu, phi, sigma, blocks, tau) {
  n_subjects <- nrow(y)
  n_time <- ncol(y)
...
#'
#' @keywords internal
.em_m_step_ad <- function(suff_stats, order, blocks, estimate_mu, 
                          n_subjects, n_time) {
  S1 <- suff_stats$S1
  S2 <- suff_stats$S2
  n <- suff_stats$n
