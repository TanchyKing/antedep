test_that("ci_ad returns Wald intervals for complete-data Gaussian AD fit", {
  set.seed(510)
  y <- simulate_ad(
    n_subjects = 90,
    n_time = 6,
  ...
  ci <- ci_ad(fit, level = 0.95, parameters = "all")

  expect_s3_class(ci, "ad_ci")
...
test_that("ci_ad rejects missing-data Gaussian AD fits", {
  set.seed(511)
  y <- simulate_ad(
    n_subjects = 60,
    n_time = 5,
  ...
  fit_em <- fit_ad(y, order = 1, na_action = "em", em_max_iter = 20)
