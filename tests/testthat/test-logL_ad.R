testthat::test_that("logL_ad matches manual computation for order 0", {
    set.seed(123)
    n <- 5
    N <- 3
...
testthat::test_that("logL_ad matches manual computation for order 1", {
    set.seed(234)
    n <- 5
    N <- 4
    y <- matrix(rnorm(n * N), nrow = n, ncol = N)
...
testthat::test_that("logL_ad matches manual computation for order 2", {
    set.seed(345)
    n <- 6
    N <- 5
    y <- matrix(rnorm(n * N), nrow = n, ncol = N)
...
testthat::test_that("logL_ad input validation behavior", {
    y <- matrix(rnorm(20), nrow = 5, ncol = 4)

    testthat::expect_error(logL_ad(y, order = 3))
    testthat::expect_error(logL_ad(y, order = 1, mu = 1:2))
    testthat::expect_error(logL_ad(y, order = 2, phi = matrix(0, nrow = 2, ncol = 2)))
    testthat::expect_error(logL_ad(y, order = 1, blocks = 1))

    testthat::expect_equal(logL_ad(y, order = 0, sigma = -1), -Inf)
...
testthat::test_that("logL_ad is higher near the generating parameters", {
    ...
    ll_true <- logL_ad(y, order = 1, mu = 0, phi = 0.6, sigma = 1)
    ll_wrong <- logL_ad(y, order = 1, mu = 0, phi = 0.1, sigma = 1)
