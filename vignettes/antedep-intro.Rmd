...
logLik(fit_gau1)
BIC(fit_gau1, fit_gau2)
bic_order_gau(y_gau, max_order = 2)
```
...
logLik(fit_inad1)
bic_order_inad(y_inad, max_order = 2, thinning = "binom", innovation = "pois")
```
...
head(confint(fit_cat1), 3)
```

```{r}
# Order tests for CAT
run_order_tests_cat(y_cat, max_order = 2)
```
