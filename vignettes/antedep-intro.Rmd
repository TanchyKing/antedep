...
Packaged datasets include `bolus_inad`, `cattle_growth`, `cochlear_implant`, and `cochlear_implant_cat`.
...
y_gau <- simulate_gau(n_subjects = 80, n_time = 6, order = 1, phi = 0.5)
...
lrt_order_gau(y_gau, p = 0, q = 1)$p_value
```
...
```{r}
y_gau_miss <- y_gau
y_gau_miss[sample(length(y_gau_miss), 24)] <- NA

fit_gau_em <- fit_gau(y_gau_miss, order = 1, na_action = "em", em_max_iter = 40)
...
lrt_one_sample_gau(y_gau, mu0 = mu0, p = 1)$p_value
lrt_two_sample_gau(y_gau, blocks = blocks_gau, p = 1)$p_value
```

```{r}
# Wald contrast test: H0 that adjacent means are equal
C <- diff(diag(ncol(y_gau)))
lrt_contrast_gau(y_gau, C = C, p = 1)$p_value
```

```{r}
# Covariance homogeneity across groups
lrt_homogeneity_gau(y_gau, blocks = blocks_gau, p = 1)$p_value
```
...
lrt_order_inad(y_inad_miss, order_null = 0, order_alt = 1, blocks = blocks_inad)$p_value
lrt_homogeneity_inad(y_inad_miss, blocks = blocks_inad, order = 1, test = "mean")$p_value
```
...
fit_cat_miss_em <- fit_cat(y_cat_miss, order = 1, na_action = "em", em_max_iter = 80)
...
lrt_order_cat(y_cat_miss, order_null = 0, order_alt = 1)$p_value
lrt_homogeneity_cat(y_cat_miss, blocks = blocks_cat, order = 1)$p_value
```
...
- CAT missing-data time-invariance/stationarity tests (`lrt_timeinvariance_cat`, `lrt_stationarity_cat`, `run_stationarity_tests_cat`) are not implemented yet.
